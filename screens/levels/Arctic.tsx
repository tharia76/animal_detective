...
export default function ArcticScreen({ onBackToMenu, backgroundImageUri, skyBackgroundImageUri }: ArcticScreenProps) {
  ...
  const fullText = useMemo(() => t('levelArctic'), [t, lang]);
    ...
    >
      ...
      <Animated.View style={[styles.fullscreenContainer, { opacity: videoFadeAnim }]} pointerEvents="box-none">
        <LevelVideoPlayer
          ...
          autoPlay={true}
          contentFit="fill"
          fallbackColor="#4a6b8b"
          onLoad={() => console.log('✅ Arctic video loaded')}
          onError={(error) => {
            console.error('❌ Arctic video error:', error);
            handleVideoEnd();
          }}
          onVideoEnd={() => {
            console.log('🏁 Arctic video ended');
            handleVideoEnd();
          }}
...
const styles = StyleSheet.create({
  ...
  levelTitleWrapper: {
    backgroundColor: 'rgba(70, 130, 180, 0.7)', // Arctic blue theme
    paddingHorizontal: 25,
    paddingVertical: 15,
    borderRadius: 50, // Pill shape
    borderWidth: 2,
    borderColor: 'rgba(135, 206, 235, 0.8)', // Sky blue border
  },
  typewriterContainer: {
    flexDirection: 'row',
    alignItems: 'center',
