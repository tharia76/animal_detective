...
export default function BirdsScreen({ onBackToMenu, backgroundImageUri, skyBackgroundImageUri }: BirdsScreenProps) {
  ...
  const fullText = useMemo(() => t('levelBirds'), [t, lang]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const saved = await AsyncStorage.getItem('animalProgress_birds');
        if (saved) {
    ...
    >
      ...
      <View style={styles.fullscreenContainer}>
        <LevelVideoPlayer
          ...
          onLoad={() => console.log('✅ Birds video loaded')}
          onError={(error) => {
            console.error('❌ Birds video error:', error);
            handleVideoEnd();
          }}
          onVideoEnd={() => {
            console.log('🏁 Birds video ended');
            handleVideoEnd();
          }}
...
const styles = StyleSheet.create({
  ...
  levelTitleWrapper: {
    backgroundColor: 'rgba(30, 144, 255, 0.7)', // Dodger blue sky theme
    paddingHorizontal: 25,
    paddingVertical: 15,
    borderRadius: 50, // Pill shape
    borderWidth: 2,
    borderColor: 'rgba(173, 216, 230, 0.8)', // Light blue border
  },
  typewriterContainer: {
    flexDirection: 'row',
    alignItems: 'center',
