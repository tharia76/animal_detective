...
export default function OceanScreen({ onBackToMenu, backgroundImageUri, skyBackgroundImageUri }: OceanScreenProps) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const saved = await AsyncStorage.getItem('animalProgress_ocean');
        if (saved) {
...
const styles = StyleSheet.create({
  ...
  levelTitleWrapper: {
    backgroundColor: 'rgba(25, 25, 112, 0.7)', // Midnight blue ocean theme
    paddingHorizontal: 25,
    paddingVertical: 15,
    borderRadius: 50, // Pill shape
    borderWidth: 2,
    borderColor: 'rgba(64, 224, 208, 0.8)', // Turquoise border
  },
  typewriterContainer: {
    flexDirection: 'row',
    alignItems: 'center',
