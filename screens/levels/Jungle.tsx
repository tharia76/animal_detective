...
export default function JungleScreen({ onBackToMenu, backgroundImageUri, skyBackgroundImageUri }: JungleScreenProps) {
  ...
  const fullText = useMemo(() => t('levelJungle'), [t, lang]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const saved = await AsyncStorage.getItem('animalProgress_jungle');
        if (saved) {
    ...
    >
      ...
      <View style={styles.fullscreenContainer}>
        <LevelVideoPlayer
          ...
          onLoad={() => console.log('✅ Jungle video loaded')}
          onError={(error) => {
            console.error('❌ Jungle video error:', error);
            handleVideoEnd();
          }}
          onVideoEnd={() => {
            console.log('🏁 Jungle video ended');
            handleVideoEnd();
          }}
        />
...
const styles = StyleSheet.create({
  ...
  levelTitleWrapper: {
    backgroundColor: 'rgba(34, 139, 34, 0.7)', // Jungle green theme
    paddingHorizontal: 25,
    paddingVertical: 15,
    borderRadius: 50, // Pill shape
    borderWidth: 2,
    borderColor: 'rgba(50, 205, 50, 0.8)', // Lime green border
  },
  typewriterContainer: {
    flexDirection: 'row',
    alignItems: 'center',
